...
export async function POST(request: Request) {
  if (!(await getLeemingAdmin())) {
    return Response.json({ error: "仅 LEEMING 管理员可上传文件" }, { status: 403 });
  }
  ...
  if (file.size > 500 * 1024 * 1024) {
    return Response.json({ error: "文件不能超过 500 MB" }, { status: 413 });
  }

  const key = `resources/${Date.now()}-${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
